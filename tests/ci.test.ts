...
describe("repository hygiene examples", () => {
  ...
  it("keeps SECURITY.md aligned with the implemented auth and logging model", () => {
    ...
    for (const text of [
      ...
      "`upload_image` may fetch a caller-provided remote image URL before uploading it to Substack.",
      "must not point at `localhost` or private network address ranges",
